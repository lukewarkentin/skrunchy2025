...
ka <- read_xlsx(
  here("data-raw/kitsumkalum/kitsumkalum-escapement-by-age-1984-2020.xlsx"),
  skip = 1
)
# get just total
kat <- ka[, c(1, 12:15)]
names(kat) <- c("y", "4", "5", "6", "7")
K_star_old_df <- pivot_longer(
  kat,
  cols = 2:5,
  names_to = "a",
  values_to = "K_star"
...
a_new <- read.csv(here(
  "data-raw",
  "kitsumkalum",
  "kitsumkalum-age-observations-by-sex-2021-on.csv"
))
...
# Get K_star for 2021-2024
K_star_new <- get_K_star(
  K = ks$kitsumkalum_escapement,
  y_K = ks$year,
  K_M = ks$male_escapement,
  K_F = ks$female_escapement,
  omega_KM = omega_KM[ 1:4, ],
  omega_KF = omega_KF[ 1:4, ],
  H = H[names(H) %in% 2021:2024],
  H_star = H_star[as.character(2021:2024), ]
)
K_star_new
