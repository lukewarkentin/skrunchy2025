...
bd_old <- readRDS(here(
  "data-raw/tyee-data-1984-2020/tyee-biodata-age-sex-length-CU-1984-2020.rda"
))
...
d2021_res <- read_xlsx(
  here("data-raw", "tyee-genetics", "gsi-results-include-jacks", file),
  sheet = res_tab
)
d2021_ext <- read_xlsx(
  here("data-raw", "tyee-genetics", "gsi-results-include-jacks", file),
...
md <- merge(
  bd1,
  gsi1,
  by.x = c("scale_book_scale", "YEAR (CATCH)"),
  by.y = c("Vial", "CatchYear"),
  all.x = TRUE
)
...
omega <- get_omega(
  aarr[c("Skeena", pops_keep), , c("4", "5", "6", "7")],
  save_csv = FALSE
)
omega$omega[,, "7"]
...
omega_J <- get_omega(
  aarr[c("Skeena", pops_keep), , c("3", "4", "5", "6", "7")],
  save_csv = FALSE
)
omega_J$omega[,, "7"]
...
usethis::use_data(omega, omega_J, n_age_observations, overwrite = TRUE)
