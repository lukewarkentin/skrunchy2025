...
d <- read_xlsx( here( "data-raw/freshwater-catch.xlsx"), range = "A1:P47", .name_repair = fix_names)

names(d)
# rename variables
names(d) <- sub( "tyee_catch_jacks_plus_large", "tyee", names(d) )

names(d) <- sub("fw_sport_catch_large_plus_jacks_above_terrace", "rec_catch_U", names(d) )
...
vars_keep <- c( "year", "tyee", "rec_catch_U", "rec_catch_L", "rec_release_L", "FN_catch_L", "FN_catch_U")
